...
void main() {
  test('dashboard response is parsed with safe defaults', () {
    final summary = DashboardSummary.fromJson({'total_members': 100, 'living_members': 80});
  ...
  test('tree response maps nodes and edges', () {
    final tree = FamilyTree.fromJson({
      'nodes': [{'uuid': 'root', 'name': 'Ahmad', 'position': {'x': 10, 'y': 20}}],
      'edges': [{'source_uuid': 'root', 'target_uuid': 'child'}],
    });
