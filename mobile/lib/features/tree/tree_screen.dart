      ...
      Padding(
        ...
        child: TextField(
          decoration: const InputDecoration(
              labelText: 'UUID anggota pusat', suffixIcon: Icon(Icons.search)),
          onSubmitted: (value) =>
              ref.read(currentMemberUuidProvider.notifier).state = value.trim(),
        ),
      ),
      Expanded(
          child: ref.watch(treeProvider).when(
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final edge in tree.edges) {
      ...
      if (source != null && target != null) {
        canvas.drawLine(Offset(source.x + 75, source.y + 36),
            Offset(target.x + 75, target.y + 36), paint);
      }
    }
  }
