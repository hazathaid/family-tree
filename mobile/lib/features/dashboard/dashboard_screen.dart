  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.all(16), children: [
        families.when(
          ...
          data: (items) {
            if (selected == null && items.isNotEmpty) {
              Future.microtask(() => ref.read(currentFamilyProvider.notifier).state = items.first);
            }
            return DropdownButtonFormField<Family>(
              value: selected,
              decoration: const InputDecoration(labelText: 'Keluarga'),
              items: items.map((family) => DropdownMenuItem(value: family, child: Text(family.name))).toList(),
              onChanged: (family) => ref.read(currentFamilyProvider.notifier).state = family,
            );
          },
        ),
        ...
        Text('Aktivitas Keluarga', style: Theme.of(context).textTheme.titleLarge),
        ...
        ref.watch(timelineProvider).when(
          ...
          data: (items) => items.isEmpty
              ...
              : Column(children: items.map((item) => ListTile(
...
class _Stat extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SizedBox(width: 156, child: Card(child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(children: [Icon(icon, color: Theme.of(context).colorScheme.primary),
          Text('$value', style: Theme.of(context).textTheme.headlineMedium), Text(label, textAlign: TextAlign.center)]),
      )));
}
...
class _Empty extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.all(24),
      child: Column(children: [const Icon(Icons.family_restroom, size: 48), const SizedBox(height: 8), Text(message)]));
}
